...
CONFIG="--config=noaws --config=nogcp --config=nohdfs --config=nonccl --config=numa --distinct_host_configuration=false --config=mkl_open_source_only"
...
$BAZEL_BIN build ${CONFIG} ${CONFIG_CC} ${CONFIG_CPP} ${CONFIG_BAZEL} --action_env=fcc_ENV="-Nclang -Kfast -Knolargepage" --action_env=FCC_ENV="-Nclang -Kfast -Knolargepage" //tensorflow/tools/pip_package:build_pip_package
